...
class TestResultsClass(unittest.TestCase):
    ...
    def setUpClass(cls) -> None:
        ...
        results_dir = os.path.join(
            rootpath.detect(),
            "experiments/directed/data/undirected_comp/wfg1_2obj_3dim"
    ...
    def test_hpv(self):
        """
        test dominated hypervolume computation
        """
...
class TestResultsContainerClass(unittest.TestCase):
    ...
    def setUpClass(cls) -> None:
        cls.results_dir = os.path.join(
            ...
            "OF_objective_function__opt_DirectedSaf__ninit_10__surrogate"
            "_MultiSurrogateGP__ei_False__target_0p35_3p14__w_0p5")
        assert os.path.isdir(cls.results_dir)

        cls.reference_dir = os.path.join(
            rootpath.detect(),
            'experiments/directed/data/undirected_comp/wfg1_2obj_3dim/log_data'
...
#
    def test_isntantiation_methods(self):
        ...
        self.container.save("./exemple_ResultsContainer.pkl")
        rc3 = ResultsContainer("./exemple_ResultsContainer.pkl")
        for i in range(len(self.container.results)):
            np.testing.assert_equal(self.container.results[i].__dict__,
