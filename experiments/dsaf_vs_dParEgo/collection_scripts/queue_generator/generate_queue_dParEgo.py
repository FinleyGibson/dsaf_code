...
targets = TARGETS_D[target_name.lower()]
...
opt_opts = {'dparego': "DirectedParEgo(objective_function=objective_function, "
                       "ei=True,  s=5, rho=0.05, target=t, limits=limits, "
...
for seed, t in remaining_configs:
    exec('optimisers += [{}]'.format(opt_opts['dparego']))
