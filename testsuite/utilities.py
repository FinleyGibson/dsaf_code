...
DIR_DIRECTED_RESULTS = os.path.join(
    rootpath.detect(), "experiments/data/saf_directed"
)
DIR_DIRECTED_REFERENCE = os.path.join(
    rootpath.detect(), "experiments/data/saf_undirected"
)
...
if __name__ == "__main__":
    ...
    a = np.arange(0, 2.01, 0.01)
    x = np.vstack((a, 2-a)).T
    # t = np.array([[0.8, 1.0], [1.4, 0.5]]).reshape(2, -1)
    t = np.array([[1.2, 1.2], [0.9, 1.3]]).reshape(2, -1)

    doms = dominates(t, x, maximize=True)

    fig = plt.figure()
    ax = fig.gca()
    ax.scatter(*x[doms].T, c="C3", s=5)
    ax.scatter(*x[np.logical_not(doms)].T, c="C0", s=5)
