...
def draw_samples(func, n_obj, n_dim, n_samples, random=False, bar=True):
    """
    draws random samples from the wfg function specified by func, n_obj
    and n_dim

    :param func: wfg.WFG{n}
        wfg function
    :param n_obj: int
    ...
    :param random: bool
        if False draw randomly from the Pareto front, if True randomly
        sample the parameter space.
    :param bar: bool
        if True show the tqdm loading bar, otherwise do not
    :return: (np.array, np.array)
        x and y values of samples
    """
    ...
    if random:
        ...
        for n in tqdm(range(n_samples), disable=not bar):
...
def get_target_dict():
    """
    quick helper to load target dict from
    experiments/directed/collection_scripts/targets/targets
    :return: dict
        dictionary of targets
    """
    target_path = os.path.join(
        rootpath.detect(),
        "experiments/dsaf_vs_saf/collection_scripts/targets/targets")
    with open(target_path, "r") as infile:
        D_targets = json.load(infile)
    return {k.lower(): np.asarray(v) for k,v in D_targets.items()}


def get_dhv_refpoint_dict():
    """
    quick helper to load dual hypervolume refpoint  dict from
    experiments/directed/collection_scripts/targets/targets
    :return: dict
        dictionary of targets
    """
    dhv_path = os.path.join(
        ...
        "dual_hypervolume_processing/dual_hypervolume_pareto_refpoints.json")
    with open(dhv_path, "r") as infile:
        D_ref_points = json.load(infile)
    return {k.lower(): np.asarray(v[1]) for k,v in D_ref_points.items()}


def get_dual_hypervolume_refvol_points(problem, n_target):
    """
    quick helper to load dual hypervolume monte carlo volume points dict
    from experiments/direc
    :return: dict
        dictionary of targets
    """
...
def get_igd_refpoint_dict():
    """
    quick helper to load igd reference  points dict from
    experiments/dsaf_vs_saf/analysis/processing_scripts/igd_reference_points
    :return: dict
        dictionary of reference points indexed by problem name
    """
    rp_path = os.path.join(
        rootpath.detect(),
        "experiments/dsaf_vs_saf/analysis/processing_scripts/"
        "igd_reference_points/igd_reference_points.json")
    with open(rp_path, "r") as infile:
        D_igd_rps = json.load(infile)
    return {k.lower(): np.asarray(v) for k, v in D_igd_rps.items()}


def map_target_to_target_n(target):
    """
    takes target name and finds its index in the dictionary returned by
    get_target_dict()
    :param target: np.array/list
    :return: int
    """
    ...
    for k, v in D.items():
        if target in v:
            sorted_targets0 = v[:3][np.argsort(v[:3, 0])]
            sorted_targets1 = v[3:][np.argsort(v[3:, 0])]
            assert np.all(sorted_targets0==v[:3]) or np.all(sorted_targets1==v[3:])
            return int(np.where(np.all(target == v, axis=1))[0])

if __name__ == "__main__":
    map_target_to_target_n([1., 2., 3.])
